...
export const recruitNewGovernor = async (colonyId) => {
  ...
  // add populations to colony objects
  // add rules for governors and populations
  await setActivityLog(
    `<p class="activityText recruitGovernor">Recruiting new governor.</p>`
  );
};

export const changeGovernorStatus = async () => {
  if (
    eventState.interval - eventState.lastGovernorStatusUpdate >
    intervalState.govStatus
  ) {
    ...
    await Governor();
    await setActivityLog(
      `<p class="activityText governorStatus">${governor.name} has become ${
