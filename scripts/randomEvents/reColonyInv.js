    ...
    for (const inv of colonyInv) {
      ...
      if (specRaid) {
        if (inv.mineralId === favoredMineral) {
          ...
          await editColonyInventory(invId, obj);
          colonyInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
            inv.colony_stock * multiplier.fav
          )}T - \t${mineralName}.</li>`;
        } else {
          const obj = {
          ...
          await editColonyInventory(invId, obj);
          colonyInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
            inv.colony_stock * multiplier.spec
          )}T - \t${mineralName}.</li>`;
        }
      } else {
        const obj = {
          colony_stock:
            inv.colony_stock - Math.floor(inv.colony_stock * multiplier.reg),
        };
        await editColonyInventory(invId, obj);
        colonyInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
          inv.colony_stock * multiplier.reg
        )}T - \t${mineralName}.</li>`;
      }
    }
    colonyInvHTML += "</ul></p>";
