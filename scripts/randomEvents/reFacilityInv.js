...
export const facilityMineralLoss = async (type, id, name, ratio, pirate) => {
  ...
  const specRaid = facilityInv.some(
    (item) => item.mineralId === favoredMineral
  );
  let facilityInvHTML = "";
      ...
      if (specRaid) {
        if (inv.mineralId === favoredMineral) {
          const obj = {
            facility_stock:
              inv.facility_stock -
              Math.floor(inv.facility_stock * multiplier.fav),
          };
          await editFacilityInventory(invId, obj);
          facilityInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
            inv.facility_stock * multiplier.fav
          )}T - \t${mineralName}.</li>`;
        } else {
          const obj = {
            facility_stock:
              inv.facility_stock - inv.facility_stock * multiplier.spec,
          };
          await editFacilityInventory(invId, obj);
          facilityInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
            inv.facility_stock * multiplier.spec
          )}T - \t${mineralName}.</li>`;
        }
      } else {
        const obj = {
          facility_stock:
            inv.facility_stock - inv.facility_stock * multiplier.reg,
        };
        await editFacilityInventory(invId, obj);
        facilityInvHTML += `<li class="activityText securityLosses pirateRaid">${Math.floor(
          inv.facility_stock * multiplier.reg
        )}T - \t${mineralName}.</li>`;
      }
    }
