...
export const changeFacilityStatus = async () => {
  ...
  // if (
  //   eventState.interval - eventState.lastFacilityStatusUpdate >
  //   intervalState.facilityStatus
  // ) {
  ...
  await setActivityLog(
    `<p class="activityText facilityStatus">${facility.name} is ${
      obj.is_active ? "now online." : "offline."
    }.</p>`
  );
...
export const addFacilityMinerals = async () => {
  ...
  for (const inv of facilityInventory) {
    if (inv.facilityId === facilityId && inv.facility.is_active) {
      ...
      await setActivityLog(
        `<p class="activityText facilityOffline">Production halted on ${inv.facility.name}.</p>`
      );
      return;
    }
  }
...
export const facilityAttack = async (facilityId, attackType, pirate) => {
  ...
  pirateHTML += await calculateSecurityLosses(
    attackType,
    facilityId,
    facility.name,
    facility.security,
  ...
  pirateHTML += await facilityMineralLoss(
    attackType,
    facilityId,
    facility.name,
    forceRatio,
